...
def scrape(hyperlink):
    ...
    Returns:
        a tuple containing a response object containing the scraped content and
        a Boolean identifying whether the scraping was successful"""

    # Scrape the website and record if scraping was successful
    try:
        response = requests.get(hyperlink)
        success = True
        return response, success
    # If an error is raised during the request process, tag this as a failed 
    # request and return an empty string for the response
    except (
...
def preprocess_text(text):
    ...
    # Access stop word dictionary without loading it via nltk, which would
    # require loading a large library into the server and waste storage space
